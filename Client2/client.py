...
def MakeConfigFile():
    globalValues['username'] = "test1"
    globalValues['password'] = "test1"
...
def checkNewFilesInServer():
    ...
    for file in files_missing:
        ...
        with open(globalValues['sync_dir']+"/.tmp/hash.txt", 'r') as fHash:
            hash = fHash.read().encode('latin-1')

        print(hash)
        print("---------------------------------")
        print(Encclass.fileHash(file+".zip"))

        if hash == Encclass.fileHash(file+".zip"):
            ...
            os.remove(globalValues['sync_dir']+".tmp/hash.txt")
            return
